<?
  ...
  start_time();
  $r = mysql_query("SELECT eq_log.i_id, eq_log.q_id, 
    i_name, l_mar, l_mqr, l_an, l_qn, eq_authors.a_id, a_id2, 
    l_time, l_correct, l_total, l_time, l_ip, l_session, a_name, q_rating, u_name
    FROM eq_log 
    LEFT JOIN eq_quotes USING (q_id) 
    LEFT JOIN eq_authors ON (eq_authors.a_id=eq_log.a_id)
    LEFT JOIN eq_isles ON (eq_log.i_id=eq_isles.i_id)
    LEFT JOIN cc_users ON (eq_log.u_id=cc_users.u_id)
