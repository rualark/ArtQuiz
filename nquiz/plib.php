<?
  function show_player($ts, $w_id=0, $p=0, $autoplay=1, $time=0, $loop="true", $time2=0) {
...
<?
  ...
  if ($autoplay>0) echo "ready: function() { $(\"#jquery_jplayer_1\").jPlayer(\"play\", $time); },";
?>
    swfPath: "jplayer/js",
		supplied: "mp3",
		wmode: "window",
...
<?
  //autoPlay: true,
  //if ($autoplay>0) echo "window.setTimeout(function () { $('#jquery_jplayer_1').jPlayer('play', '$time'); }, 100);";
  $time = $time + 0;
?>
...
<?
  if ($time2>0) {
?>
    var myImg = document.getElementById('nquiz_progressbar');
    myImg.style.width = <? echo "Math.max(0, (x - $time)/($time2-$time)*$jpeg_width)"; ?> + 'px';
<?
  }
  if ($time>0) echo "if (x < 1) if (jpData.status.paused == 0) $('#jquery_jplayer_1').jPlayer('play', $time);";
?>
  }, jp_freq);
  $("#jquery_jplayer_1").bind($.jPlayer.event.play, function (event) {
    ...
    $.each(playlist, function (index, obj) {
      if (index == current) {
<?
  //if ($time>0) echo "if ((jp_play_count == 1) || (event.jPlayer.status.currentTime < 1)) $('#jquery_jplayer_1').jPlayer('play', $time);";
?>
...
<?
  //if ($w_id>0) echo "$('.jp_current_track_title').html('<a href=works.php?time=' + Math.round(event.jPlayer.status.currentTime) + '&w_id=$w_id&p=".($p+1)."><font size=+3>Next</font></a>');";
?>
