<?
  ...
  if ($act == 'search') {
    ...
    if ($cond != '') {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        echo "<tr>";
        echo "<td>";
        echo "<a href=$url_main/rlit/authors.php?a_id=$w[a_id]>";
    ...
    if ($cond != '') {
      ...
      for ($i=0; $i<$n; $i++) {
        ...
        echo "<tr>";
        echo "<td>";
        echo "<a href=$url_main/work/rlit/showtraum.php?rq_id=$rq_id&act=showauthor&tr_id=$w[tr_id]>";
  ...
  $q = "SELECT COUNT(*) as cnt FROM rq_authors";
  $r = mysql_query($q);
  echo mysql_error();
  $w = mysql_fetch_assoc($r);
  $q = "SELECT COUNT(*) as cnt FROM rq_quotes";
  $r = mysql_query($q);
  echo mysql_error();
  $w2 = mysql_fetch_assoc($r);
  echo "<table width=100%>";
  echo "<td><b>Фильтр:";
  echo "<td align=right><font color=gray>Всего $w[cnt] авторов, $w2[cnt] цитат. <a href=authors.php?act=search>Поиск</a>";
  echo "</table>";
  ...
  $q = "SELECT * FROM rq_authors WHERE 1=1 $cond ORDER BY $order LIMIT 1000";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  ...
  for ($i=0; $i<$n2; $i++) {
    ...
    if ($old_group != $w2[a_group]) {
      ...
      echo "<option value='' style='color: #aaaaaa'>$w2[a_group]</option>";
