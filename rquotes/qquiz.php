<?
  ...
  $an = $_SESSION[rq_an];
  $qn = $_SESSION[rq_qn];
  $i_id = $_SESSION[rq_i_id];
  $q_id = $_SESSION[rq_q_id];
  //$a_id = $_SESSION[a_id];
  $total = $_SESSION[rq_total];
  $correct = $_SESSION[rq_correct];
  $state = $_SESSION[rq_state];
  $wa = $_SESSION[rq_wa];
  $logged = $_SESSION[rq_logged];
  $q_hist = $_SESSION[rq_q_hist];
  ...
  if (strlen($wi[i_at])>1) {
    ...
    $join .= "LEFT JOIN rq_qtags USING (q_id) ";
  }
  //echo $cond;
  ...
  function save_session() {
    $_SESSION[rq_mar] = $GLOBALS[mar];
    $_SESSION[rq_mqr] = $GLOBALS[mqr];
    $_SESSION[rq_an] = $GLOBALS[an];
    $_SESSION[rq_qn] = $GLOBALS[qn];
    $_SESSION[rq_i_id] = $GLOBALS[i_id];
    $_SESSION[rq_q_id] = $GLOBALS[q_id];
    //$_SESSION[a_id] = $GLOBALS[a_id];
    $_SESSION[rq_l] = $GLOBALS[l];
    $_SESSION[rq_total] = $GLOBALS[total];
    $_SESSION[rq_correct] = $GLOBALS[correct];
    $_SESSION[rq_mr] = $GLOBALS[mr];
    $_SESSION[rq_state] = $GLOBALS[state];
    $_SESSION[rq_wa] = $GLOBALS[wa];
    $_SESSION[rq_logged] = $GLOBALS[logged];
    $_SESSION[rq_q_hist] = $GLOBALS[q_hist];
  }
  ...
  echo "<b>$wi[i_name]</b> ($an авторов, $qn цитат).";
  ...
  if ($a_id>0) {
    ...
    if ($wq2[q_img] != 0) echo "<a target=_blank href='works.php?q_id=$wq2[q_id]'><img src=qpic/$wq2[a_id]/$wq2[q_id].jpg></a><hr>";
      ...
      echo "<table><td width=700><a target=_blank href='works.php?q_id=$wq2[q_id]'><font size=+2><pre>$wq2[q_txt]</pre></font></table><hr>";
    }
    // Show tags
    $r2 = mysql_query("SELECT * FROM rq_qtags WHERE q_id='$wq2[q_id]'");
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    echo mysql_error();
    if ($n2>0) echo "Метки: ";
    ...
    if ($n2>0) echo "<hr>";
    if ($wq2[a_id] == $a_id) echo "<font color=green><b>Правильно!</b></font>";
    else echo "<font color=red><b>Неправильно!</b></font> ";
    echo "<br>";
    ...
    if ($wq2[rl_id]>0) {
      ...
      $r3 = mysql_query("SELECT * FROM rl_works WHERE a_id='$wq2[rl_id]' ORDER BY RAND() LIMIT 1");
      echo mysql_error();
      $w3 = mysql_fetch_assoc($r3);
      if ($w3[w_id]>0) {
        echo "<a target=_blank href=$url_main/rlit/works.php?w_id=$w3[w_id]>$w3[w_name]</a><br>";
      }
    }
    if ($wq2[rp_id]>0) {
      echo "<tr><td bgcolor=#eeeeee>";
      $r3 = mysql_query("SELECT * FROM l_works WHERE a_id='$wq2[rp_id]' ORDER BY RAND() LIMIT 1");
      echo mysql_error();
      $w3 = mysql_fetch_assoc($r3);
      if ($w3[w_id]>0) {
        echo "<a target=_blank href=$url_main/epquiz/works.php?w_id=$w3[w_id]>$w3[w_name]</a><br>";
        $sa = explode("\r", $w3[w_text]);
        $c = count($sa);
        for ($i=0; $i<$c; $i++) {
          if ($i > $max_lines) {
            echo "<a target=_blank href=$url_main/pquiz/works.php?w_id=$w3[w_id]>...</a><br>";
            break;
          }
          echo $sa[$i]."<br>";
        }
    ...
    //$a_id = 0;
    if (($logged == 0) && ($q_id>0)) {
      mysql_query("INSERT INTO rq_log 
        VALUES('', '$a_id', '$wq2[a_id]', '$q_id', NOW(), '$this_correct', '".get_ip()."', '".session_id()."', '$total', '$i_id', '$mar', '$mqr', '$an', '$qn', '$ua[u_id]')");
      echo mysql_error();
      if ($i_id>0) {
        mysql_query("UPDATE rq_isles SET i_games=i_games+1 WHERE i_id='$i_id'");
        echo mysql_error();
      }
    ...
    if (count($ha)>0) echo "В вашей истории ".count($ha)." цитат.";
    exit;
  }
  echo "</form>";
  $logged = 0;
  ...
  if (($state == 0) || ($q_id == 0)) {
    ...
    if (strlen($wi[i_at])>1) {
      $q = "SELECT * FROM (SELECT rq_authors.* FROM rq_atags 
        STRAIGHT_JOIN rq_authors ON (rq_atags.a_id=rq_authors.a_id) 
        WHERE a_rating<='$mar' $cond 
        GROUP BY rq_authors.a_id 
        ORDER BY RAND() 
        LIMIT 5) a ORDER BY a_year1";
    } else {
      $q = "SELECT * FROM (SELECT * FROM rq_authors 
        WHERE a_rating<='$mar' $cond 
        ORDER BY RAND() 
        LIMIT 5) a ORDER BY a_year1";
    }
    ...
    for ($i=0; $i<$n; $i++) {
      ...
      if (strlen($wa[$i][a_desc])>$max_desc) $wa[$i][a_desc] = mb_substr($wa[$i][a_desc], 0, $max_desc)."...";
      // Select random work
      $r3 = mysql_query("SELECT q_id FROM rq_quotes WHERE a_id='".$wa[$i][a_id]."' ORDER BY RAND() LIMIT 1");
      echo mysql_error();
      $w3 = mysql_fetch_assoc($r3);
      $wa[$i][q_id] = $w3[q_id];
    ...
    $q = "SELECT * FROM rq_quotes $join WHERE q_rating<='$mqr' AND a_id='".$wa[$cor_ind][a_id]."' AND q_id != '".$wa[$cor_ind][q_id]."' $cond2 ORDER BY RAND() LIMIT 1";
    //echo " $q<br>";
    $r = mysql_query($q);
    echo mysql_error();
    ...
    if ($wq[q_id] == 0) {
      $q = "SELECT * FROM rq_quotes $join WHERE q_rating<='$mqr' AND a_id='".$wa[$cor_ind][a_id]."' ORDER BY RAND() LIMIT 1";
      $r = mysql_query($q);
      echo mysql_error();
      $wq = mysql_fetch_assoc($r);
      $reselected = 1;
    ...
    $r = mysql_query("SELECT * FROM rq_quotes WHERE q_id='$q_id'");
    echo mysql_error();
    $wq = mysql_fetch_assoc($r);
  }
  ...
  echo "<table><td width=700><p onmousedown='return false;' onselectstart='return false;' style='-webkit-user-select: none;
    ...
    -ms-user-select: none;
    -o-user-select: none;
    user-select: none;'><font style='font-weight: normal' size=+2><pre>$wq[q_txt]</pre>";
  /*
  $sa = explode(" ", $wq[q_txt]);
  for ($x=0; $x<count($sa); $x++) {
    if ($x>0) echo " ";
    echo "<a target=_blank href='http://www.multitran.ru/c/m.exe?CL=1&s=$sa[$x]&l1=1'>$sa[$x]</font></a>";
  }
  */
  echo "</font></p></table>";
  
  echo "<p><b>Угадайте автора цитаты:</b><br>";
