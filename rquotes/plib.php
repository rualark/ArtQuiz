<?
  ...
  function block_url($t_id, $b_id) {
    GLOBAL $url_blocks, $us;
    return "$url_blocks/$t_id/".str_pad($b_id, 3, '0', STR_PAD_LEFT).".mp3";
  }
  ...
  function show_player($t_id, $b_id, $oneblock, $symlink, $autoplay, $select=1, $loop="true") {
    GLOBAL $ts, $block_len, $us, $wbida;
    //echo $select;
  // <link href="jplayer/css/jPlayer.css" rel="stylesheet" type="text/css" />
  echo '<link href="jplayer/css/jPlayer.css" rel="stylesheet" type="text/css" />';
?>
<!--[if lt IE 9]>
  ...
  if ($oneblock) {
    ...
    if ($wbc>0) {
      for ($i=$wbc-1; $i>=0; $i--) {
        ...
        show_block($wtrain[t_id], $wtrain[b_lid], $oneblock, $symlink);
      }
    }
  } else {
...
?>
  ...
  setInterval(function() {
    ...
    //div.innerHTML = d;
    if ((x>0) && (x<1) && (jp_start_time==0)) {
      jp_start_time = n;
    }
    if (jp_start_time>0) {
      var p = n - jp_start_time;
      if (p<2000) {
        jp.jPlayer("volume", p/2000)
      } else {
        jp.jPlayer("volume", 1);
        jp_start_time = 0;
      }
    }
    if ((x>=d-2.5) && (x<d-1) && (jp_stop_time==0)) {
      jp_stop_time = n;
    }
    if (jp_stop_time>0) {
      var p = n - jp_stop_time;
      if (p<=2000) {
        jp.jPlayer("volume", Math.pow(1-p/2000, 2))
      } else {
        jp.jPlayer("volume", 0);
        jp_stop_time = 0;
      }
...
?>
  ...
  $(document).keydown(function (event) {
    ...
    if (event.keyCode == 56) $("#jquery_jplayer_1").jPlayer("play", document.currentTime - 6);
    if (event.keyCode == 57) $("#jquery_jplayer_1").jPlayer("play", document.currentTime + 6);
    if (event.keyCode == 49) {
