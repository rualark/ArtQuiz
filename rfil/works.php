<?
  ...
  if ($tag != '') {
    //load tag
    $r = mysql_query("SELECT * FROM rq_tdesc WHERE t_name='$tag'");
    //echo "SELECT * FROM rq_tags LEFT JOIN rq_tdesc USING (t_name) WHERE t_name='$tag'";
    echo mysql_error();
    ...
    if ($wt[t_parent] != '') echo "<p>Входит в группу: <a href='?tag=$wt[t_parent]'>$wt[t_parent]</a>";
    $r = mysql_query("SELECT * FROM rq_tdesc WHERE t_parent = '$tag'");
    echo mysql_error();
    $n = mysql_numrows($r);
    if ($n > 0) {
      echo "<p>Дочерние направления: ";
      for ($i=0; $i<$n; $i++) {
        $w = mysql_fetch_assoc($r);
        if ($i>0) echo ", ";
        echo "<a href='?tag=$w[t_name]'>$w[t_name]</a>";
      }
    }
    if ($wt[t_desc] != '') echo "<p><i>$wt[t_desc]</i>";
    echo "<p>";
    $r = mysql_query("SELECT * FROM rq_authors LEFT JOIN rq_atags USING (a_id) WHERE a_quotes>0 AND t_name='$tag' AND t_type>0 ORDER BY t_type desc, a_year1");
    echo mysql_error();
    $n = mysql_numrows($r);
