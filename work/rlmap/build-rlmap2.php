<?
  ...
  secure_variable("a_ename");
  secure_variable("a_ename2");
  ...
  if (($move_x != 0) && ($move_x != $map_x1)) {
    ...
    $q = "SELECT *,IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa)+map{$mi}_year+map{$mi}_ayear AS my FROM rl_authors";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
  ...
  if (isset($map_pos)) {
    ...
    $q = "UPDATE rl_authors SET map{$mi}_pos='$map_pos', a_rating='$a_rating', map{$mi}_right='$map_right', 
      a_ename='$a_ename', a_ename2='$a_ename2', map{$mi}_name='$map_name', a_country='$a_country', 
  ...
  if (isset($a_id)) {
    ...
    echo "<a href='$url_main/rlit/authors.php?a_id=$w[a_id]'>$w[a_ename]</a> [$w[a_year1]]";
    if ($w[a_wiki] == '') $w[a_wiki] = "http://en.wikipedia.org/w/index.php?search=".awiki($w[a_ename]);
    ...
    echo "<p>Name: <input size=80 name=a_ename value='$w[a_ename]'>";
    echo "<p>Name2: <input size=80 name=a_ename2 value='$w[a_ename2]'>";
  ...
  imagettfstroketext($im, $font_size+5, 0, $legend_x+1350, $legend_y+515, $cl[white], $cl[abrown], $font2, "Colors legend:", 1);
  imagettfstroketext($im, $font_size+5, 0, $legend_x+1345, $legend_y+687, $cl[white], $cl[abrown], $font2, "Shapes legend:", 1);
  $nat_legend = array(
    'USA' => 'USA', 
    'CAN' => 'Canada', 
    'RUS' => 'Russia', 
  ...
  */
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    if ($wa[$i][a_wiki] == '') $wa[$i][a_wiki] = "http://en.wikipedia.org/w/index.php?search=".awiki($wa[$i][a_ename]);
  ...
  */
  ...
  for ($i=0; $i<$n2; $i++) {
    ...
    draw_box($w2["map{$mi}_x1"], $w2["map{$mi}_y1"], $w2["map{$mi}_x2"], $w2["map{$mi}_y2"], 
      $w2["map{$mi}_color"], $w2[t_name], $w2["map{$mi}_name"], $w2["map{$mi}_label"], $w2["map{$mi}_active"], 
 ...
 // Show artists
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $y = y($year)+$lh/2+1;
    $x = $z2+40;
    $name = "$w[a_ename2]"; // $w["map{$mi}_ayear"]
    ...
    $area_dev .= make_area($x11, $y1, $x2, $y2, 
      ...
      "$w[a_ename]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]\nMY: ".$w["map{$mi}_year"]."/".$w["map{$mi}_ayear"]."\nMX: ".$w["map{$mi}_pos"], "target=_blank");
  ...
  fps("<p>Similar styles have the same color.");
  fps("<p>Most styles have a thick colored line at their side, which shows the main interval when the style was active.");
  fps("<p>Click on poet, style or country to find out more and read books (in Russian only).");
  fps("<p>Most well known authors are marked bold.");
