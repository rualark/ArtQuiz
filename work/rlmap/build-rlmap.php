<?
  ...
  function lookup_pos($st) {
    if ($st == '') return 0;
    // replace variables
    $q = "SELECT a_name, map_pos FROM rl_authors WHERE map_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
  ...
  function lookup_year($st) {
    ...
    $q = "SELECT IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa) AS ayear, 
      a_name, map_pos, a_year1, a_year2, map_year, map_ayear FROM rl_authors WHERE map_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    $w2 = mysql_fetch_assoc($r2);
    ...
    return round($w2[ayear]+$w2[map_year]+$w2[map_ayear]);
  }
  
  if (($move_x != 0) && ($move_x != $map_x1)) {
    $move_x = $move_x - $map_x1;
    // Check for collisions
    $q = "SELECT *,IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa)+map_year+map_ayear AS my FROM rl_authors";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
    ...
    for ($x=0; $x<$n2; $x++) {
      ...
      if ($p1==1) {
        ...
        mysql_query("UPDATE rl_authors SET map_pos=map_pos+($move_x) WHERE a_id='$w2[a_id]'");
        echo mysql_error();
      }
    }
    // Move tag
    $q = "UPDATE rl_tdesc SET map_x1=map_x1+($move_x), map_x2=map_x2+($move_x)
      WHERE t_name='$t_name'";
    //die($q);
    $r = mysql_query($q);
    echo mysql_error();
    //exit;
    die ("<script language=\"javascript\">location.replace(\"build-rlmap.php\");</script>"); // #t$t_name
  }
  ...
  if (isset($map_pos)) {
    ...
    $q = "UPDATE rl_authors SET map_pos='$map_pos', a_rating='$a_rating', map_right='$map_right', 
      a_name='$a_name', a_name2='$a_name2', map_name='$map_name', a_country='$a_country', 
      map_comment='$map_comment', map_year='$map_year', map_ayear='$map_ayear', map_img='$map_img' WHERE a_id='$a_id'";
    echo $q;
    $r = mysql_query($q);
    echo mysql_error();
    die ("<script language=\"javascript\">location.replace(\"build-rlmap.php\");</script>"); // #a$a_id
  }
  ...
  if (isset($a_id)) {
    ...
    $q = "SELECT a_name, map_pos FROM rl_authors WHERE map_pos>0";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
    ...
    echo "<input readonly type=text size=80 name=map_ayear value='$w[map_ayear]'><br>";
    ...
    echo "<p>Map name: <input size=80 name=map_name value='$w[map_name]'>";
  ...
  if (isset($map_x2)) {
    ...
    //die("$map_x1 $map_x2");
    // change
    $q = "UPDATE rl_tdesc SET map_x1='$map_x1', map_x2='$map_x2', map_y1='$map_y1', map_y2='$map_y2', 
  ...
  function make_nation_col($st) {
    ...
    if (strpos($st, "FRE") !== FALSE) $cl="blue";
  ...
  function draw_box($x1, $y01, $x2, $y02, $col, $text, $short, $pos, $act, $wiki, $min, $max, $mint, $maxt, $poly) {
    ...
    $area_dev .= "<area href='build-rlmap.php?t_name=$text' shape=rect coords='$x11,$by1,$bx2,$by2'>";
    ...
    */
    ...
    $area_dev .= make_area($x11, $by1, $bx2, $by2, "build-rlmap.php?t_name=$text", $add);
    if ($wiki != '') $text = $wiki;
    ...
    if ($min != 0) {
      ...
      if ($mint == 1) {
        imagefilledrectangle($im, $ax, $ay1+0,  $ax+2, $ay1+0+1,  $col_txt);
        imagefilledrectangle($im, $ax, $ay1+4,  $ax+2, $ay1+4+1,  $col_txt);
        imagefilledrectangle($im, $ax, $ay1+8,  $ax+2, $ay1+8+1,  $col_txt);
        imagefilledrectangle($im, $ax, $ay1+12, $ax+2, $ay1+12+1,  $col_txt);
        imagefilledrectangle($im, $ax, $ay1+16, $ax+2, $ay1+16+1, $col_txt);
        $ay1 += 20;
      }
  ...
  //draw_cent(1600);
  //draw_cent(1650);
  draw_cent(1400, 1425, 1933);
  draw_cent(1700, 870, 1933);
  //draw_cent(1750);
  draw_cent(1800, 2672, 870);
  draw_cent(1850, 2672, 870);
  draw_cent(1900, 2672, 870);
  draw_cent(1950, 390, 1179);
  draw_cent(2000, 1179, 2672, 1933, 390);
  ...
  */
  ...
  $q = "SELECT *,
    ...
    FROM rl_authors WHERE map_pos!=0 ORDER BY a_year1";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
  ...
  */
  ...
  // Show tags
  $q = "SELECT * FROM rl_tdesc WHERE map_x2!=0";
  $r = mysql_query($q);
  echo mysql_error();
  $n2 = mysql_numrows($r);
  for ($i=0; $i<$n2; $i++) {
 ...
 // Show artists
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $year = $w[ayear] + $w[map_year] + $w[map_ayear];
    //if ($year > 2016) $year = 2016;
    $y = y($year)+$lh/2+1;
    $x = $z2+40;
    $name = "$w[a_name2]"; // $w[map_ayear]
    if ($w[map_name] != '') $name = "$w[map_name]";
    if ($w[map_comment] != '') $name .= " - $w[map_comment]";
    $bbox = imageftbbox($font_size, 0, $fnt, $name);
    if ($w[map_right] == 1) $x = $x - $bbox[4] - 10;
    $x1 = $x-5;
    ...
    if (is_array($abox)) foreach ($abox as $key => $val) {
      ...
      if (($val[y1] == $y1) && ($val[x1] >= $x1) && ($val[x2] <= $x2)) $found=1;
      if ($found) $intersec .= "Intersection $w[a_name2] - $key<br>";
    }
    ...
    $area_main .= make_area($x1, $y1, $x2, $y2, 
      "<?=\$url_main;?>/rlit/authors.php?mp=$w[map_pos]&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]");
    // Dev areas
    $x11 = round(($x1+$x2)/2);
    $area_dev .= make_area($x1, $y1, $x11, $y2, 
      "$url_main/rlit/authors.php?mp=$w[map_pos]&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]\nMY: $w[map_year]/$w[map_ayear]\nMX: $w[map_pos]");
    $area_dev .= make_area($x11, $y1, $x2, $y2, 
      "build-rlmap.php?mp=$w[map_pos]&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]\nMY: $w[map_year]/$w[map_ayear]\nMX: $w[map_pos]", "target=_blank");
    ...
    if ($w[map_img] != '') {
      ...
      $area_main .= make_area($x2+2, $y2+2, $x2+2+$ii[0], $y2+2+$ii[1], 
        "<?=\$url_main;?>/rlit/works.php?tag=".$img2tname[$w[map_img]], 
        $img2tname[$w[map_img]]);
  ...
  imagefttext($im, $font_size, 0, 1355, $im_height-10, $cl[black], "fonts/arial.ttf", "This timeline was created by Alexey Arkhipenko (c) 2017");
  // Save map
