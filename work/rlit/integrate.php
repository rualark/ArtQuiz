<?
  ...
  exit;
  // Find l_authors
  $q = "SELECT * FROM rl_authors WHERE a_art>0"; 
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    ...
    if ($w[rp_id] == 0) {
      ...
      if ($n2 == 1) {
        ...
        mysql_query("UPDATE rl_authors SET rp_id='$w2[a_id]' WHERE a_id='$w[a_id]'");
        echo mysql_error();
        mysql_query("UPDATE rl_authors SET ep_id='$w2[ep_id]' WHERE a_id='$w[a_id]' AND ep_id=0");
        echo mysql_error();
        mysql_query("UPDATE l_authors SET rl_id='$w[a_id]' WHERE a_id='$w2[a_id]'");
        echo mysql_error();
        mysql_query("UPDATE ep_authors SET rl_id='$w[a_id]' WHERE a_id='$w2[ep_id]'");
        echo mysql_error();
      }
      if ($n2 > 1) {
        for ($x=0; $x<$n2; $x++) {
    ...
    if (($w[eq_id] == 0) && ($w[a_ename] != '')) {
      ...
      if ($n2 == 1) {
        $w2 = mysql_fetch_array($r2);
        echo "$w[a_name] = $w2[a_name]<br>";
        mysql_query("UPDATE rl_authors SET eq_id='$w2[a_id]' WHERE a_id='$w[a_id]'");
        echo mysql_error();
        if (($w[ep_id] == 0) && ($w2[ep_id] != 0)) {
          mysql_query("UPDATE rl_authors SET ep_id='$w2[ep_id]' WHERE a_id='$w[a_id]'");
          echo mysql_error();
          //echo "Updated ep_id!<br>";
        }
        mysql_query("UPDATE l_authors SET rl_id='$w[a_id]' WHERE ep_id='$w2[a_id]' AND rl_id=0");
        //echo mysql_error();
        mysql_query("UPDATE eq_authors SET rl_id='$w[a_id]' WHERE a_id='$w2[eq_id]'");
        echo mysql_error();
      }
      if ($n2 > 1) {
        for ($x=0; $x<$n2; $x++) {
    ...
    if ($w[ep_id] != 0) {
      ...
      if (($w2[eq_id] != 0) && ($w[eq_id] == 0)) {
        ...
        //mysql_query("UPDATE rl_authors SET eq_id='$w2[eq_id]' WHERE a_id='$w[a_id]'");
      }
      if (($w2[eq_id] == 0) && ($w[eq_id] != 0)) {
      ...
      if (($w2[ra_id] != 0) && ($w[rp_id] == 0)) {
        ...
        //mysql_query("UPDATE rl_authors SET rp_id='$w2[ra_id]' WHERE a_id='$w[a_id]'");
      }
      if (($w2[eq_id] == 0) && ($w[eq_id] != 0)) {
...
Wrong:
Лесли Поулс Хартли = Lindsay Hartley
Евгений Шварц = Elizabeth Robinson Schwartz
