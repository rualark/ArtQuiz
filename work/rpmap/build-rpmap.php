<?
  ...
  function lookup_pos($st) {
    // replace variables
    $q = "SELECT a_name, map_pos FROM l_authors WHERE map_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating DESC, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
  ...
  function lookup_year($st) {
    // replace variables
    $q = "SELECT a_name, map_pos, a_year1, a_year2, map_year FROM l_authors WHERE map_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating DESC, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    $w2 = mysql_fetch_assoc($r2);
  ...
  if (($move_x != 0) && ($move_x != $map_x1)) {
    ...
    $q = "SELECT *,ROUND((IF(a_year2=0, 2016, a_year2)+a_year1)/2+map_year) as my FROM l_authors";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
      ...
      if (($p1==0) && ($p2==1)) die("<font color=red>Error: this move will include new artist $w2[a_name]</font>");
    }
    // Move artists
    $q = "SELECT *,ROUND((IF(a_year2=0, 2016, a_year2)+a_year1)/2+map_year) as my FROM l_authors";
    $r2 = mysql_query($q);
    ...
    for ($x=0; $x<$n2; $x++) {
      ...
      if ($p1==1) {
        ...
        mysql_query("UPDATE l_authors SET map_pos=map_pos+($move_x) WHERE a_id='$w2[a_id]'");
        echo mysql_error();
      }
    }
    // Move tag
    $q = "UPDATE l_tdesc SET map_x1=map_x1+($move_x), map_x2=map_x2+($move_x)
      WHERE t_name='$t_name'";
    //die($q);
    $r = mysql_query($q);
    echo mysql_error();
    //exit;
    die ("<script language=\"javascript\">location.replace(\"build-rpmap.php#t$t_name\");</script>"); 
  }
  ...
  if (isset($map_pos)) {
    ...
    $q = "UPDATE l_authors SET map_pos='$map_pos', a_rating='$a_rating', map_right='$map_right', 
      a_name='$a_name', a_name='$a_name', a_country='$a_country', 
      map_comment='$map_comment', map_year='$map_year', map_img='$map_img' WHERE a_id='$a_id'";
    $r = mysql_query($q);
    echo mysql_error();
    die ("<script language=\"javascript\">location.replace(\"build-rpmap.php#a$a_id\");</script>"); 
  }
  ...
  if (isset($a_id)) {
    ...
    $q = "SELECT * FROM l_authors WHERE a_id='$a_id'";
    $r = mysql_query($q);
    echo mysql_error();
    ...
    echo "<a href='$url_main/pquiz/authors.php?a_id=$w[a_id]'>$w[a_name]</a> [$w[a_year1]]";
    if ($w[a_wiki] == '') $w[a_wiki] = "http://ru.wikipedia.org/w/index.php?search=".awiki($w[a_name]);
    echo " <a target=_blank href=\"$w[a_wiki]\"><img height=20 border=0 src=i/wikipedia.jpg></a> ";
    $q2 = "SELECT t_name, t_id FROM l_atags WHERE a_id=".$w[a_id]."";
    $r2 = mysql_query($q2);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
      $w2 = mysql_fetch_assoc($r2);
      if ($x > 0) echo ", ";
      echo "<a href=$url_main/pquiz/works.php?t_id=$w2[t_id]>$w2[t_name]</a>";
    }

    echo "<p>Map position: ";
    echo "<input id=sel1 type=text size=80 name=map_pos value='$w[map_pos]' list=bcomlist><br>";
    echo "<datalist id=bcomlist>";
    $q = "SELECT a_name, map_pos FROM l_authors WHERE map_pos>0";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
  ...
  if (isset($map_x2)) {
    ...
    //die("$map_x1 $map_x2");
    // change
    $q = "UPDATE l_tdesc SET map_x1='$map_x1', map_x2='$map_x2', map_y1='$map_y1', map_y2='$map_y2', 
      map_label='$map_label', map_active='$map_active', 
      t_year1='$t_year1', t_year2='$t_year2', t_year1t='$t_year1t', t_year2t='$t_year2t',
      map_color='$map_color', t_wiki='$t_wiki' WHERE t_name='$t_name'";
    // t_name='$t_name2' 
    //die($q);
    $r = mysql_query($q);
    echo mysql_error();
  ...
  if (isset($t_name)) {
    echo "<body onload='document.getElementById(\"sel1\").focus(); document.getElementById(\"sel1\").setSelectionRange(0, document.getElementById(\"sel1\").value.length)'>";
    $q = "SELECT * FROM l_tdesc WHERE t_name='$t_name'";
    $r = mysql_query($q);
    echo mysql_error();
    $w = mysql_fetch_array($r);
    echo "<form>";
  ...
  function make_nation_bgcol($st) {
    ...
    if (strpos($st, "KOR") !== FALSE) $cl="lblue2";
    if (strpos($st, "ISL") !== FALSE) $cl="lorange";
  ...
  function draw_box($x1, $y1, $x2, $y2, $col, $text, $pos, $act, $wiki, $min, $max, $mint, $maxt) {
    ...
    $x11 = round(($bx1+$bx2)/2);
    /*
    $area_main .= "<area href='<?=\$url_main;?>/pquiz/artists.php?tag=$text' shape=rect coords='$bx1,$by1,$bx2,$by2'>";
    $area_dev .= "<area href='build-rpmap.php?t_name=$text' shape=rect coords='$x11,$by1,$bx2,$by2'>";
    $area_dev .= "<area target=_blank href='http://ru.wikipedia.org/w/index.php?search=$text' shape=rect coords='$bx1,$by1,$x11,$by2'>";
    */
    $area_main .= make_area($bx1, $by1, $bx2, $by2, "<?=\$url_main;?>/pquiz/works.php?tag=$text");
    $area_dev .= make_area($x11, $by1, $bx2, $by2, "build-rpmap.php?t_name=$text");
    if ($wiki != '') $text = $wiki;
    $area_dev .= make_area($bx1, $by1, $x11, $by2, "http://ru.wikipedia.org/w/index.php?search=$text");
    // show active years
    if ($min != 0) {
      $ay1 = y($min);
      $ay2 = y($max);
  ...
  // Count density
  $q = "SELECT a_year1, a_year2 FROM l_authors WHERE a_rating>100";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
  ...
  $acnt[1200/$cent_step] = 0;
  $acnt[1300/$cent_step] = 0;
  $acnt[1400/$cent_step] = 1;
  $acnt[1500/$cent_step] = 1;
  $acnt[1600/$cent_step] = 1;
  $acnt[1700/$cent_step] = 4;
  ...
  $acnt[1800/$cent_step] = 22;
  $acnt[1900/$cent_step] = 22;
  ...
  $legend_x = -135;
  $legend_y = -465;
  $im_scroll = imagecreatefromgif('i/scroll6.gif');
  imagecopy($im, $im_scroll, $legend_x+1196, $legend_y+470, 0, 0, 280, 362);

  imagettfstroketext($im, $font_size+5, 0, $legend_x+1300, $legend_y+550, $cl[white], $cl[abrown], $font2, "Легенда", 1);
  $nat_legend = array('USA' => 'США', 'ITA' => 'Италия', 'FRA' => 'Франция', 'RUS' => 'Россия', 
    'DEU' => 'Германия', 'ENG' => 'Англия', 'SPA' => 'Испания', 'JAP' => 'Япония', 
    'GRC' => 'Греция', 'IND' => 'Индия', '' => 'Другие');
  $q = "SELECT *, COUNT(*) as cnt FROM l_authors WHERE map_pos>0 AND a_rating>0 GROUP BY a_country ORDER BY cnt desc";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  $x = $legend_x+1250;
  ...
  foreach ($nat_legend as $key => $val) {
    ...
    $y2 = $y-$lh-1;
    if ($nat != 'Other') {
      $area_main .= make_area($x1, $y1, $x2, $y2, "<?=\$url_main;?>/pquiz/authors.php?order=a_country,%20a_year1");
      $area_dev .= make_area($x1, $y1, $x2, $y2, "$url_main/pquiz/authors.php?order=a_country,%20a_year1");
    }
    $y += 23;
    if ($y > $legend_y+720) {
      $x += 110;
  ...
  $area_main .= make_area($x2+2, $y2, $x2+2+150, $y2+5+$ii[1], 
    "<?=\$url_main;?>/pquiz/works.php?tag=".$img2tname[$img], 
    $img2tname[$img]);
  ...
  $q = "SELECT a_year1, a_year2, a_id, a_name, a_name2, a_rating, a_country, a_wiki,
    ...
    FROM l_authors WHERE map_pos>0 AND a_rating>0 ORDER BY a_year1";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    ...
    if ($wa[$i][a_wiki] == '') $wa[$i][a_wiki] = "http://ru.wikipedia.org/w/index.php?search=".awiki($wa[$i][a_name]);
    $wa[$i][a_years] = $wa[$i][a_year1]."-";
    if ($wa[$i][a_year2] > 0) $wa[$i][a_years] .= $wa[$i][a_year2];
    // Load tags
    $q2 = "SELECT t_name FROM l_atags WHERE a_id=".$wa[$i][a_id]."";
    $r2 = mysql_query($q2);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
  ...
  */
  ...
  // Show tags
  $q = "SELECT * FROM l_tdesc WHERE map_x2!=0";
  $r = mysql_query($q);
  echo mysql_error();
  $n2 = mysql_numrows($r);
  for ($i=0; $i<$n2; $i++) {
    $w2 = mysql_fetch_array($r);
    draw_box($w2[map_x1], y($w2[map_y1]), $w2[map_x2], y($w2[map_y2]), 
      $w2[map_color], $w2[t_name], $w2[map_label], $w2[map_active], 
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $area_main .= make_area($x1, $y1, $x2, $y2, 
      "<?=\$url_main;?>/pquiz/authors.php?mp=$w[map_pos]&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[t_names]\n$w[a_country]\n$w[a_years]");
    // Dev areas
    $x11 = round(($x1+$x2)/2);
    ...
    $area_dev .= make_area($x11, $y1, $x2, $y2, 
      "build-rpmap.php?mp=$w[map_pos]&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[t_names]\n$w[a_country]\n$w[a_years]");
    ...
    if ($w[map_img] != '') {
      ...
      $area_main .= make_area($x2+2, $y2+2, $x2+2+$ii[0], $y2+2+$ii[1], 
        "<?=\$url_main;?>/pquiz/works.php?tag=".$img2tname[$w[map_img]], 
        $img2tname[$w[map_img]]);
  ...
  imagefttext($im, $font_size, 0, 1055, $im_height-10, $cl[black], "fonts/arial.ttf", "This timeline was created by Alexey Arkhipenko (c) 2016");
  // Save map
  ...
  fps("<p>Кликните на поэта или стиль чтобы узнать больше или прочитать стихи. Самые известные поэты выделены жирным шрифтом.");
  fps("<p>Используйте Ctrl+F или 'Найти на странице' для поиска стилей и поэтов на карте.");
  fps_dev("
