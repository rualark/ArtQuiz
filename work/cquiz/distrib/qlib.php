<?
  ...
  function block_url($t_id, $b_id, $symlink=0) {
    GLOBAL $block_path, $symlink_path, $us, $fsep;
  ...
  function full_url() {
    GLOBAL $block_path, $symlink_path, $us, $fsep, $ts;
  ...
  function block_url2($t_id, $b_id, $symlink=0) {
    ...
    else return "$block_path$fsep$t_id$fsep".str_pad($b_id, 3, '0', STR_PAD_LEFT).".mp3";
  }
  ...
  function update_system($key, $val) {
    ...
    echo mysql_report_error();
    write_log("log/cquiz-system.log", date("Y-m-d H:i:s")." ".$_SERVER['REMOTE_ADDR']." ".$_SERVER['PHP_SELF']." Update system $key $val");
  }
  ...
  function show_composers($su_id=0) {
    // su_id shows that following user if >0
    GLOBAL $us, $ua, $ts, $level_cust, $comp_hint, $uname, $debug, $ca_selected;
    // Create randoms
    ...
    for ($i=0; $i<$n; $i++) {
      ...
      $cpic = "<a target=_blank href=am.php?u_id=$us[u_id]&c_id=$w[c_id]><img align=left title='$w[c_name]' border=0 width=$width1 src=cpic/$w[c_id].jpg></font></a>";
  ...
  function log_game($b_id, $sec, $c_id2, $ok, $wrong2, $cent) {
    ...
    if ($ts[t_id] == $us[t_id]) {
      ...
      $w = mysql_fetch_assoc($r);
      // build query
      $q = "INSERT INTO cc_qlog VALUES('','$us[u_id]','$w[nw]','$sec',$us[u_oneblock],$us[u_allcomp],$us[u_level],$us[u_trating],$ts[c_id],$ts[t_id],'$b_id','$ok','$wrong2','$c_id2','$cent','$r_rd1','$ncust2','".$_SERVER['REMOTE_ADDR']."',x'$cab',x'$gab',0,0,'".($us[r_total]+1)."','$ncust');";
  ...
  function stop_time($st="", $show=1) {
    ...
    // Write to views
    $q = "INSERT INTO cc_views VALUES(NOW(), '$us[u_id]', '$_SERVER[REMOTE_ADDR]', '$_SERVER[HTTP_HOST]', '$_SERVER[SCRIPT_NAME]', '$_SERVER[QUERY_STRING]', '$dif', '$view_child')";
    //echo $q;
    mysql_query2($q);
    echo mysql_report_error();
