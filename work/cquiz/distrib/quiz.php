<?
  ...
  if (isset($c_id)) {
    ...
    if ($su_id==0) {
      if ($repeated == 0) $r1 = log_game($b_id, time()-$start, $c_id, $ok, $wrong2, $cent);
    ...
    if ($us[t_id] != 0) {
      ...
      $b_id = $us[b_id];
      $start = $us[u_start];
      if ($su_id==0) echo "<font color=red>Unfinished question detected. Please answer the question to continue.</font>";
      // Create links again
      if ($play_symlinks>0) {
