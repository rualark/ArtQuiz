<?
  ...
  if ($a_id != 0) {
    ...
    $r2 = mysql_query("SELECT * FROM eq_authors WHERE a_id='$a_id2'");
    echo mysql_error();
    ...
    // Move quotes
    $q = "UPDATE eq_quotes SET a_id='$a_id' WHERE a_id='$a_id2'";
    echo "$q<br>";
    mysql_query($q);
    echo mysql_error();
    if ($w[rq_id] == 0) {
      $q = "UPDATE eq_authors SET rq_id=$w2[rq_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    if ($w[rl_id] == 0) {
      $q = "UPDATE eq_authors SET rl_id=$w2[rl_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    if ($w[rp_id] == 0) {
      $q = "UPDATE eq_authors SET rp_id=$w2[rp_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    if ($w[ep_id] == 0) {
      $q = "UPDATE eq_authors SET ep_id=$w2[ep_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    if ($w[aq_id] == 0) {
      $q = "UPDATE eq_authors SET aq_id=$w2[aq_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    if ($w[cc_id] == 0) {
      $q = "UPDATE eq_authors SET cc_id=$w2[cc_id] WHERE a_id='$a_id'";
      echo "$q<br>";
      mysql_query($q);
      echo mysql_error();
    }
    $q = "DELETE FROM eq_authors WHERE a_id='$a_id2'";
    echo "$q<br>";
    mysql_query($q);
    echo mysql_error();
    exit;
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $q = "SELECT * FROM eq_authors 
      WHERE a_id != $w[a_id] AND (
      a_name LIKE BINARY '$letter% $surname'
      OR a_name LIKE BINARY '$surname %$letter%'
      OR a_name = '$surname' OR a_name LIKE '%$w[a_name]%'
      OR ($cond))
    ";
    //$wiki = mysql_real_escape_string($w[a_wiki]);
    //$q = "SELECT * FROM eq_authors WHERE a_id > $w[a_id] AND a_wiki='$wiki'";
    //echo "$q<br>";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
      ...
      echo "$w[a_wiki] ";
      echo "<a href=$url_main/equotes/authors.php?a_id=$w[a_id] target=_blank>$w[a_name]</a> <a target=_blank href='https://www.google.com/search?q=$w[a_name]'><img style='opacity: 0.3' valign=middle height=14 src=i/google.png></a> $w[a_year1] - ";
      echo "<a href=$url_main/equotes/authors.php?a_id=$w2[a_id] target=_blank>$w2[a_name]</a> <a target=_blank href='https://www.google.com/search?q=$w2[a_name]'><img style='opacity: 0.3' valign=middle height=14 src=i/google.png></a> $w2[a_year1]";
