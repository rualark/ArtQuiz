<?
  ...
  echo "Full text: ";
  // count mr
  $r = mysql_query("SELECT w_rating FROM rl_works WHERE w_fil=1 AND (t_len>0 OR t_len2>0)
  ...
  mysql_query("UPDATE rl_authors SET a_fil=0");  
  echo mysql_error();
  $r = mysql_query("SELECT a_name, rl_works.a_id, a_fil FROM rl_works 
    LEFT JOIN rl_authors USING (a_id) 
    WHERE w_fil=1 AND (t_len>0 OR t_len2>0) GROUP BY rl_works.a_id
  ");
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    $w = mysql_fetch_assoc($r);
    mysql_query("UPDATE rl_authors SET a_fil=1 WHERE a_id='$w[a_id]'");
    echo mysql_error();
  }
