<?
  ...
  function draw_box($x1, $y1, $x2, $y2, $col, $col2, $text, $pos, $act, $wiki, $min, $max, $mint, $maxt) {
    ...
    $font_size=10;
    $btext .= "<a name='t$text'></a><font color=white size=3 style='position: absolute; top: ".($y1-15)."px; left: ".($im_width)."px'>$text</font>\n";
  ...
  for ($i=-40000/$cent_step; $i<3000/$cent_step; $i++) {
    $astart[$i] = $pos;
    $pos += round($acnt[$i]*14);
  }
  ksort($acnt);
  ...
  imagecopy($im, $im_scroll, 1196, 470, 0, 0, 320, 414);

  // Show legend
  imagettfstroketext($im, $font_size+5, 0, 1290, 550, $cl[white], $cl[abrown], $font2, "Color legend", 1);
  ...
  for ($i=0; $i<count($nat_legend); $i++) {
    ...
    $y2 = $y-$lh-1;
    if ($nat != 'Other') {
      $area_main .= "<area target=_blank title='$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]' href='<?=\$url_main;?>/aquiz/artists.php?nat=$nat' shape=rect coords='$x1,$y1,$x2,$y2'>\n";
      $area_dev .= "<area target=_blank title='$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]' href='$url_main/aquiz/artists.php?nat=$nat' shape=rect coords='$x1,$y1,$x2,$y2'>\n";
    }
    $y += 23;
    if ($y > 820) {
      $x += 120;
  ...
  $dx = 25;
  imagefilledrectangle($im, 1258, 418, 1450, 452, $cl[green]);
  imagerectangle($im, 1258, 418, 1450, 452, $cl[gray]);
  imagefttext($im, $font_size+5, 0, 1266, 443, $cl[white], $font2, "WATCH TUTORIAL");
  $area_dev .= "<area target=_blank href='i/tutorial.png' shape=rect coords='1258,418,1450,452'>\n";
  $area_main .= "<area target=_blank href='i/tutorial.png' shape=rect coords='1258,418,1450,452'>\n";
  
  // Load artists
  $q = "SELECT a_start, a_stop, a_id, a_years, a_name, a_name2, a_rating, a_genre, a_nation, a_wiki_en,
    map_comment, map_pos, map_right, map_year, ROUND((IF(a_stop=0, 2016, a_stop)+a_start)/2) as my
    FROM aq_artists WHERE map_pos>0 AND a_rating>0 ORDER BY my";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
  ...
  */
  ...
  $map_pos = array(
    ...
    'Ruisdael' => 4,
    'Hooch' => 4,
    'Native American art' => -30,
    'Chinese art XVII-XIX' => 20,
    'Borisov-Musatov' => 5,
    'Serov' => 6,
    'Newman' => 2,
    'Siqueiros' => 1,
  ...
  $ngroup = array(
    array('Italian', 'French'),
    array('Dutch', 'German', 'British', 'Spanish', 'Belgian', 'Lithuanian', 'Armenian', 'Czech', 'Flemish', 'Greek'),
    array('Chinese', 'Japanese', 'Korean', 'Indian', 'Islamic', 'Russian'),
    array('American', 'Mexican'),
  );
  for ($k=0; $k<count($ngroup); $k++) {
    for ($i=0; $i<$n; $i++) {
      $w = &$wa[$i];
      if ($w[map_pos] == -1) continue;
      $good = 0;
      foreach ($ngroup[$k] as $key => $nat) {
        //echo "$w[a_nation] $nat<br>";
        if (strpos($w[a_nation], $nat) !== false) $good = 1;
      }
      if ($good == 0) continue;
      if ($z > $gpos[$k][1]) $z = $gpos[$k][0];
      //if (($w[my] > 1350) && ($w[my] < 1600) && ($z > 1000)) $z = 1;
      $fnt = $font;
      if ($w[a_rating]>2) $fnt = $font2;
      ...
      $year2 = $w[a_stop];
      if ($year2 == 0) $year2 = 2016;
      $year = ($year1+$year2)/2 + $map_pos[$w[a_name2]];
      ...
      $area_main .= "<area target=_blank title='$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]' href='<?=\$url_main;?>/aquiz/artist.php?mp=$w[map_pos]&a_id=$w[a_id]&g=$w[a_genre]&y=$w[a_start]' shape=rect coords='$x1,$y1,$x2,$y2'>\n";
      // Dev areas
      $x11 = round(($x1+$x2)/2);
      $area_dev .= "<area target=_blank title='$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]' href='$url_main/aquiz/artist.php?mp=$w[map_pos]&a_id=$w[a_id]&g=$w[a_genre]&y=$w[a_start]' shape=rect coords='$x1,$y1,$x11,$y2'>\n";
      $area_dev .= "<area title='$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]' href='build-amap.php?mp=$w[map_pos]&a_id=$w[a_id]&g=$w[a_genre]&y=$w[a_start]' shape=rect coords='$x11,$y1,$x2,$y2'>\n";
      $btext .= "<a name=a$w[a_id]></a><font color=white size=3 style='position: absolute; top: ".($y-15)."px; left: ".($im_width)."px'>$name</font>\n";
      //echo "$z - $w[a_name2]<br>";
      $z += $bbox[4]+25;
    }
  }
  ...
  fps($btext);
  fps("<img class=coords border=0 src=maps/en$id.png usemap='#Navigation'>");
  fps("</div>");
  fps("<map name=Navigation>");
  fps_main($area_main);
  fps_dev($area_dev);
  fps("</map>");
  //fps("<p><a target=_blank href='i/tutorial.png'>Here is a tutorial to this timeline</a>.");
