<?
  ...
  if (($move_x != 0) && ($move_x != $map_x1)) {
    ...
    echo mysql_error();
    //exit;
    die ("<script language=\"javascript\">location.replace(\"build-amap2.php#t$t_name\");</script>"); 
  }
  ...
  if (isset($map_pos)) {
    ...
    $r = mysql_query($q);
    echo mysql_error();
    die ("<script language=\"javascript\">location.replace(\"build-amap2.php#a$a_id\");</script>"); 
  }
  ...
  function draw_box($x1, $y1, $x2, $y2, $col, $col2, $text, $pos, $act, $wiki, $min, $max, $mint, $maxt) {
    ...
    $area_dev .= make_area($x11, $by1, $bx2, $by2, "build-amap2.php?t_name=$text");
    if ($wiki != '') $text = $wiki;
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $area_dev .= make_area($x11, $y1, $x2, $y2, 
      "build-amap2.php?mp=$w[map_pos]&a_id=$w[a_id]&g=$w[a_genre]&y=$w[a_start]", 
      "$w[a_name]\n$w[a_genre]\n$w[a_nation]\n$w[a_years]");
