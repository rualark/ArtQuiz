<?
  ...
  function lookup_tag($st, $par) {
    ...
    if ($st == '') return 0;
    // replace variables
    $q = "SELECT t_name, map{$mi}_$par FROM rq_tdesc WHERE map{$mi}_x1!=0 AND t_name LIKE '%$st%' ORDER BY LENGTH(t_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
  ...
  function lookup_pos($st) {
    ...
    if ($st == '') return 0;
    // replace variables
    $q = "SELECT a_name, map{$mi}_pos FROM rq_authors WHERE map{$mi}_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    $w2 = mysql_fetch_assoc($r2);
  ...
  function lookup_year($st) {
    ...
    $q = "SELECT IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa) AS ayear, 
      a_name, map{$mi}_pos, a_year1, a_year2, map{$mi}_year, map{$mi}_ayear FROM rq_authors WHERE map{$mi}_pos!=0 AND a_name LIKE '%$st%' ORDER BY a_rating, LENGTH(a_name)";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    $w2 = mysql_fetch_assoc($r2);
  ...
  if ($act == 'moveall') {
    ...
    $q = "SELECT * FROM rq_tdesc WHERE map{$mi}_x2 != 0";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
  ...
  if ($act == 'moveall2') {
    ...
    $q = "SELECT * FROM rq_tdesc WHERE 1=1 $cond ORDER BY map{$mi}_x1 $desc";
    echo "$q<br>";
    $q2 = "SELECT *, IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa)+map{$mi}_year+map{$mi}_ayear AS my FROM rq_authors HAVING map{$mi}_pos!=0 $cond2";
    echo "$q2<br>";
    ...
    for ($x=0; $x<$n2; $x++) {
      ...
      //$q = "UPDATE rq_tdesc SET map{$mi}_x1=map{$mi}_x1+'$map_pos', map{$mi}_x2=map{$mi}_x2+'$map_pos' WHERE t_name='$w2[t_name]'";
      //echo "<br>$q ";
      //mysql_query("$q");
      //echo mysql_error();
    }
    ...
    for ($x=0; $x<$n2; $x++) {
      $w2 = mysql_fetch_assoc($r2);
      fputs($fh, "UPDATE rq_authors SET map{$mi}_pos=".$w2["map{$mi}_pos"]." WHERE a_id='$w2[a_id]'\r\n");
      $q = "UPDATE rq_authors SET map{$mi}_pos=map{$mi}_pos+'$map_pos' WHERE a_id='$w2[a_id]'";
      echo "<br>$q ($w2[a_name])";
      mysql_query($q);
      echo mysql_error();
    }
    */
    die ("<script language=\"javascript\">location.replace(\"build-rfmap{$id}.php\");</script>"); // #t$t_name
  }
  
  function move_tag($t_name, $move_x, $forcemove=0) {
    GLOBAL $sa, $mi, $id, $amoved;
    $q = "SELECT * FROM rq_tdesc WHERE t_name='$t_name'";
    $r2 = mysql_query($q);
    echo mysql_error();
    ...
    $q = "SELECT *,IF(a_year2!=0, (a_year1+a_year2)/2, a_year1+$sa)+map{$mi}_year+map{$mi}_ayear AS my FROM rq_authors WHERE map{$mi}_pos != 0";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
    ...
    for ($x=0; $x<$n2; $x++) {
      ...
      if ($p1==1) {
        if ($amoved[$w2[a_name]] == 0) {
          ...
          fputs($fh, "UPDATE rq_authors SET map{$mi}_pos=".$w2["map{$mi}_pos"]." WHERE a_id='$w2[a_id]'; # ".date("Y-m-d H:i:s")." - $_SERVER[HTTP_HOST] $_SERVER[SCRIPT_NAME] $_SERVER[QUERY_STRING]\r\n");
          mysql_query("UPDATE rq_authors SET map{$mi}_pos=map{$mi}_pos+($move_x) WHERE a_id='$w2[a_id]'");
          echo mysql_error();
    ...
    // Move tag
    fputs($fh, "UPDATE rq_tdesc SET map{$mi}_x1=".$wt["map{$mi}_x1"].", map{$mi}_x2=".$wt["map{$mi}_x2"]." WHERE t_name='$wt[t_name]'; # ".date("Y-m-d H:i:s")." - $_SERVER[HTTP_HOST] $_SERVER[SCRIPT_NAME] $_SERVER[QUERY_STRING]\r\n");
    $q = "UPDATE rq_tdesc SET map{$mi}_x1=map{$mi}_x1+($move_x), map{$mi}_x2=map{$mi}_x2+($move_x)
      WHERE t_name='$t_name'";
    //die($q);
    $r = mysql_query($q);
    echo mysql_error();
  ...
  if (isset($map_pos)) {
    ...
    $q = "UPDATE rq_authors SET map{$mi}_pos='$map_pos', a_rating='$a_rating', map{$mi}_right='$map_right', 
    ...
    $r = mysql_query($q);
    echo mysql_error();
    die ("<script language=\"javascript\">location.replace(\"build-rfmap{$id}.php\");</script>"); // #a$a_id
  }
  
  if (isset($a_id)) {
    echo "<body onload='document.getElementById(\"sel1\").focus(); document.getElementById(\"sel1\").setSelectionRange(0, document.getElementById(\"sel1\").value.length)'>";
    $q = "SELECT * FROM rq_authors WHERE a_id='$a_id'";
    $r = mysql_query($q);
    echo mysql_error();
    ...
    echo "<a href='$url_main/rfil/authors.php?a_id=$w[a_id]'>$w[a_name]</a> [$w[a_year1]]";
    ...
    $q2 = "SELECT t_name FROM rq_atags WHERE a_id=".$w[a_id]." AND t_type>0 ORDER BY t_type DESC";
    $r2 = mysql_query($q2);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
      $w2 = mysql_fetch_assoc($r2);
      if ($x > 0) echo ", ";
      echo "<a href=$url_main/rfil/works.php?tag=$w2[t_name]>$w2[t_name]</a>";
    }

    echo "<p>Map position: ";
    echo "<input id=sel1 type=text size=80 name=map_pos value='".$w["map{$mi}_pos"]."' list=bcomlist><br>";
    echo "<datalist id=bcomlist>";
    $q = "SELECT a_name, map{$mi}_pos FROM rq_authors WHERE map{$mi}_pos!=0";
    $r2 = mysql_query($q);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    for ($x=0; $x<$n2; $x++) {
  ...
  if (isset($map_x2)) {
    ...
    //die("$map_x1 $map_x2");
    // change
    $q = "UPDATE rq_tdesc SET map{$mi}_x1='$map_x1', map{$mi}_x2='$map_x2', map{$mi}_y1='$map_y1', map{$mi}_y2='$map_y2', 
  ...
  function draw_box($x1, $y01, $x2, $y02, $col, $text, $desc, $short, $pos, $act, $wiki, $min, $max, $mint, $maxt, $poly) {
    ...
    $area_dev .= "<area href='build-rfmap{$id}.php?t_name=$text' shape=rect coords='$x11,$by1,$bx2,$by2'>";
    ...
    */
    ...
    if ($desc != '') $add .= "\n- $desc";
    $area_main .= make_area($bx1, $by1, $bx2, $by2, "<?=\$url_main;?>/rfil/works.php?tag=$text", $add);
    $area_dev .= make_area($x11, $by1, $bx2, $by2, "build-rfmap{$id}.php?t_name=$text", $add);
    if ($wiki != '') $text = $wiki;
  ...
  $x=175;
  $y=2004;
  imagefilledrectangle($im, 21, y(2000), $im_width-1, y(2000)+3, $cl[black]);
  imagefttext($im, 11, 0, $x-145, y($y)+16, $cl[black], $font2, "Цвет направления:");
  $x = draw_box($x-8, $y, 0, $y+10, 'ared', 'Материализм', '', 'Материализм', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'ablue', 'Объективный идеализм', '', 'Объективный идеализм', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'ayellow', 'Субъективный идеализм', '', 'Субъективный идеализм', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'acyan', 'Политическая философия', '', 'Политика', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'aorange', 'Восточная философия', '', 'Восток', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'agray', 'Другие', '', 'Другие', 1, 0, '', 0, 0, 0, 0, '');
  imagefill($im, $x, y($y), $cl[lgray2]);
  ...
  $name = "Материализм";
  $x += 190;
  imagefttext($im, 11, 0, $x-145, y($y)+16, $cl[black], $font2, "Рамка философа:");
  $y = y(2011);
  ...
  $x=175;
  $y=2014;
  imagefttext($im, 11, 0, $x-145, y($y)+19, $cl[black], $font2, "Цвет философа:");
  ...
  foreach ($nat_legend as $key => $val) {
    ...
    $y2 = $y-$lh-1;
    if ($nat != 'Другие') {
      $area_main .= make_area($x1, $y1, $x2, $y2, "<?=\$url_main;?>/rfil/authors.php?country=$key");
      $area_dev .= make_area($x1, $y1, $x2, $y2, "$url_main/rfil/authors.php?country=$key");
    }
    $x += $bbox[4]+20;
    $i++;
  }
  ...
  $q = "SELECT *,
    ...
    FROM rq_authors WHERE map{$mi}_pos!=0 ORDER BY a_year1";
  $r = mysql_query($q);
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    ...
    // Load tags
    $q2 = "SELECT t_name, t_type FROM rq_atags WHERE a_id=".$wa[$i][a_id]." AND t_type>0 ORDER BY t_type DESC";
    $r2 = mysql_query($q2);
    echo mysql_error();
    $n2 = mysql_numrows($r2);
    $old_type=0;
 ...
 // Show artists
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $area_main .= make_area($x1, $y1, $x2, $y2, 
      "<?=\$url_main;?>/rfil/authors.php?mp=".$w["map{$mi}_pos"]."&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]");
    // Dev areas
    $x11 = round(($x1+$x2)/2);
    ...
    $area_dev .= make_area($x11, $y1, $x2, $y2, 
      "build-rfmap{$id}.php?mp=".$w["map{$mi}_pos"]."&a_id=$w[a_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]\nMY: ".$w["map{$mi}_year"]."/".$w["map{$mi}_ayear"]."\nMX: ".$w["map{$mi}_pos"]."\nQN: $w[a_quotes]"); //, "target=_blank"
    ...
    if ($w["map{$mi}_img"] != '') {
      ...
      $area_main .= make_area($x2+2, $y2+2, $x2+2+$ii[0], $y2+2+$ii[1], 
        "<?=\$url_main;?>/rfil/works.php?tag=".$img2tname[$w["map{$mi}_img"]], 
  ...
  fps("<p>Толстая цветная линия сбоку зоны направления показывает интервал времени, когда стиль был наиболее распространен.");
  fps("<p>Кликните на автора или стиль чтобы узнать больше или прочитать цитаты. Самые известные авторы выделены жирным шрифтом.");
  fps("<p>Используйте Ctrl+F или 'Найти на странице' для поиска направлений и авторов на карте.");
  fps_dev("
