<?
  ...
  imagefttext($im, 11, 0, $x-145, y($y)+16, $cl[black], $font2, "Zone color:");
  $x = draw_box($x-8, $y, 0, $y+10, 'ared', 'Материализм', '', 'Materialism', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'ablue', 'Объективный идеализм', '', 'Objective idealism', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'ayellow', 'Субъективный идеализм', '', 'Subjective idealism', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'acyan', 'Политическая философия', '', 'Politics', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'aorange', 'Восточная философия', '', 'East', 1, 0, '', 0, 0, 0, 0, '');
  $x = draw_box($x-8, $y, 0, $y+10, 'agray', 'Другие', '', 'Other', 1, 0, '', 0, 0, 0, 0, '');
  ...
  $name = "Materialism";
  $x += 190;
  imagefttext($im, 11, 0, $x-145, y($y)+16, $cl[black], $font2, "Philosopher shape:");
  $y = y(2011);
  ...
  $x=175;
  $y=2014;
  imagefttext($im, 11, 0, $x-145, y($y)+19, $cl[black], $font2, "Philosopher color:");
  ...
  */
  ...
  foreach ($nat_legend as $key => $val) {
    ...
    /*
    if ($nat != 'Другие') {
      $area_main .= make_area($x1, $y1, $x2, $y2, "<?=\$url_main;?>/equotes/authors.php?country=$key");
      $area_dev .= make_area($x1, $y1, $x2, $y2, "$url_main/equotes/authors.php?country=$key");
    }
    */
    $x += $bbox[4]+20;
    $i++;
 ...
 // Show artists
  ...
  for ($i=0; $i<$n; $i++) {
    ...
    $area_main .= make_area($x1, $y1, $x2, $y2, 
      "<?=\$url_main;?>/equotes/authors.php?mp=".$w["map{$mi}_pos"]."&a_id=$w[eq_id]&y=$w[a_year1]", 
      "$w[a_name]\n$w[a_prof]\n$w[t_names]\n$w[a_country]\n$w[a_years]");
    // Dev areas
    $x11 = round(($x1+$x2)/2);
  ...
  fps("<p>Dots in the name show reductions.");
  fps("<p>Similar schools have the same color.");
  fps("<p>Some schools have a thick colored line at their side, which shows the main interval when the school was active.");
  fps("<p>Click on philosopher, school or country to find out more and read quotes.");
  fps("<p>Most well known philosophers are marked bold.");
