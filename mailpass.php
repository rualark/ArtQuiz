<?
  ...
  echo "<base target=_parent />";
   ...
   if ($newp != "") {
    ...
    mysql_query("UPDATE cc_users SET u_pass=PASSWORD('$newp') WHERE u_id='$u_id'");
    echo mysql_error();
  ...
  if ($uname != "") {
    ...
    if (strtolower($ua[u_name]) == strtolower($uname)) {
      ...
      $recipients = array($ua[u_email]);
      $headers = array (
          'From' => 'noreply@artquiz.sourceforge.net',
          'To' => $ua[u_email],
          'Subject' => 'Password for ArtQuiz',
      );
      //print_r($headers);
      $body = "Please follow the link to set new password for user $ua[u_name]:\r\n\r\n".
        "$url_main/mailpass.php?u_id=$ua[u_id]&h=$ua[u_pass]\r\n\r\nYou received this mail because someone tried to recover your password. If it was not you, you can do nothing.";
