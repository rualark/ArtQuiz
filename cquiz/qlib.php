<?
  ...
  function create_isle_cst($ca) {
    GLOBAL $isle_cst, $isle_cst2, $isle_cst3, $isle_csta, $isle_cnum, $comp, $isles_max_comps;
    //load_comps();
    $ccount = get_ccount();
    $isle_cst = "";
    $isle_cst2 = "";
    ...
    for ($i=0; $i<$n; $i++) {
      ...
      $isle_csta[$id] = "<a title='$w[p_name] $w[c_country] $w[c_years]' href='am.php?c_id=$w[c_id]'><font color=#666666>$w[c_name4]</font></a>";
      $isle_cst3 .= $cst3;
  ...
  function comp_link($c_id) {
    GLOBAL $us;
    return "am.php?u_id=$us[u_id]&c_id=$c_id";
  }
  ...
  function save_user() {
    ...
    $r = mysql_query2("REPLACE INTO cc_users VALUES(
      ...
      '$us[u_total2]','$us[u_total3]', '$us[u_wbid]','$us[u_train]')");
    echo mysql_report_error();
  }
  ...
  function choose_block_new() {
    GLOBAL $us, $ts, $t_id, $level_cust, $debug, $ga_cond, $bhist_max, $min_tg_hist, $wtrain;
    // Choose composer
    $cond = "c_rating <= '$us[u_level]' AND t_count != 0";
    if ($us[u_level] == $level_cust) {
      $cond = get_ca_cond();
    }
    if ($wtrain[c_id]>0) $cond = "c_id = '$wtrain[c_id]'"; 
  ...
  function show_composers($su_id=0) {
    // su_id shows that following user if >0
    GLOBAL $us, $ua, $ts, $level_cust, $comp_hint, $uname, $debug, $ca_selected, $url_cpic;
    // Create randoms
    ...
    for ($i=0; $i<$n; $i++) {
      ...
      $cpic = "<a target=_blank href=am.php?u_id=$us[u_id]&c_id=$w[c_id]><img align=left title='$w[c_name]' border=0 width=$width1 src=$url_cpic/$w[c_id].jpg></font></a>";
      ...
      if ($us[u_select_style] == 1) {
        ...
        if ($i>5) echo " <a target=_blank href=am.php?c_id=$w[c_id]><img height=16 src=images/help.png></a> ";
  ...
  function stop_time($st="", $show=1) {
    ...
    if ($show>0) {
      echo "<p><font color=#eeeeee>The script ran ".$totaltime." seconds ";
  ...
  function show_help() {
    ...
    GLOBAL $isle_king_min;
    count_stats();
    echo "<p><font size=+2 style='BACKGROUND-COLOR: #ffccbb'>Watch video on YouTube about CQuiz: in <a target=_blank href='http://www.youtube.com/watch?v=ZVCKmIfRkL0'>English</a> or in <a target=_blank href='http://www.youtube.com/watch?v=54LvBlLe6nw'>Russian</a>.</font>";
    echo "<p><table><tr><td width=300 valign=top>";
  ...
  function show_user_king($u_id=0) {
    ...
    echo "<br>";
    echo "King rank: <a href=rating.php?level=5>".round($ua[u_kr])."</a> (<b>".($ua[u_rent]+0)."</b> renters, <b>".($ua[u_acnt]+0)."</b> answers by renters). ";
    echo "Created <b>".($ua[u_icr]+0)."</b> islands. ";
    echo "Cleared <b>".($ua[u_clears]+0)."</b> answers.<br>";
    echo "Posted <a href=forum.php>".($ua[u_posted]+0)."</a> comments, <b>".($ua[u_voted]+0)."</b> votes. Received votes: <font color=green><b>".($ua[u_fup]+0)."</b></font> ";
    echo "<img border=0 height=15 src=images/up.png> ";
    echo "<img valign=bottom border=0 height=15 src=images/down.png> ";
  ...
  function vote_compart($l_id, $v_type) { 
    ...
    if ($v_id==0) {
      ...
      mysql_query("INSERT INTO ca_votes VALUES('', '$l_id', '".get_ip()."', '$_SERVER[HTTP_USER_AGENT]', NOW(), '$v_type')");
  ...
  function show_painting($a_id, $p_id, $width, $height) {
    ...
    $r = mysql_query("SELECT * FROM paintings WHERE a_id='$a_id' AND p_id='$p_id'");
    echo mysql_error();
    $w = mysql_fetch_assoc($r);
