<?
  ...
  if (isset($t_id)) {
    ...
    $q = "SELECT * FROM nq_pdf WHERE t_id='$t_id'";
    $r = mysql_query2($q);
    echo mysql_report_error();
    if (mysql_numrows($r) > 0) { 
      $wp = mysql_fetch_array($r);
      echo "<a href=$url_main/nquiz/works.php?w_id=$wp[w_id]><img align=right src=i/sheetmusic.png width=100></a>";
    }
    ...
    // Show pictures
    $q = "SELECT * FROM ca_log LEFT JOIN aq_artists USING (a_id) WHERE t_id='$t_id' AND v_up>(v_down*2.5) ORDER BY v_up DESC LIMIT 10";
    $r = mysql_query2($q);
    echo mysql_report_error();
    $n = mysql_numrows($r);
    for ($i=0; $i<$n; $i++) {
