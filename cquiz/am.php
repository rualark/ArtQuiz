<?
  ...
  */
  ...
  echo "<a title='Pronounce name' target=_blank href=\"http://forvo.com/search/$w[c_name3]\"><img height=25 border=0 src=i/speaker.png></a> ";
  ...
  if ($w[c_amid] != '') echo "<a target=_blank href=\"http://www.allmusic.com/artist/$w[c_amid]\"><img height=20 border=0 src=i/allmusic.png></a> ";
  if ($w[c_caid] != 0) echo "<a target=_blank href=\"http://www.classicalarchives.com/composer/$w[c_caid].html\"><img height=20 border=0 src=i/classicalarchives.jpg></a> ";
  show_integration($w);
  ...
  //if (is_file("$url_cpic/$w[c_id].jpg")) 
  echo "<img width=200 src=$url_cpic/$w[c_id].jpg align=right>";
  $table = "cc_tstat2";
  if ($u_id>0) {
  ...
  */
  ...
  echo "<br><center><b>Associated paintings:</b>";
  // Show pictures
  $q = "SELECT * FROM ca_log LEFT JOIN aq_artists USING (a_id) WHERE c_id='$c_id' AND v_up>(v_down*2.5) ORDER BY v_up DESC LIMIT 10";
  $r = mysql_query2($q);
  echo mysql_report_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    ...
    echo "<img align=top style='margin-left: auto; margin-right: auto; text-align: center; display: block; max-width: 100%; max-height: 100%;' src=$img_url></a><br><a target=_blank href=$url_main/aquiz/artist.php?a_id=$w2[a_id]>$w2[a_name2]</a> ".strtoupper(substr($w2[a_nation], 0, 3))."-".(substr($w2[a_years], 0, 4))."<br>".$w2[a_genre];
