<?
  ...
  $mr = $_SESSION[rl_mr];
  $i_id = $_SESSION[rl_i_id];
  $w_id = $_SESSION[rl_w_id];
  //$a_id = $_SESSION[a_id];
  $l = $_SESSION[rl_l];
  $total = $_SESSION[rl_total];
  $correct = $_SESSION[rl_correct];
  $state = $_SESSION[rl_state];
  $wa = $_SESSION[rl_wa];
  $ay = $_SESSION[rl_ay];
  $logged = $_SESSION[rl_logged];
  $p_hist = $_SESSION[rl_p_hist];
  $mode = $_SESSION[rl_mode];
  $cur_page = $_SESSION[rl_cur_page];
  ...
  if ($i_id>0) {
    $r = mysql_query("SELECT * FROM rl_isles WHERE i_id='$i_id'");
    echo mysql_error();
  ...
  function save_session() {
    $_SESSION[rl_mr] = $GLOBALS[mr];
    $_SESSION[rl_i_id] = $GLOBALS[i_id];
    $_SESSION[rl_w_id] = $GLOBALS[w_id];
    //$_SESSION[a_id] = $GLOBALS[a_id];
    $_SESSION[rl_l] = $GLOBALS[l];
    $_SESSION[rl_total] = $GLOBALS[total];
    $_SESSION[rl_correct] = $GLOBALS[correct];
    $_SESSION[rl_mr] = $GLOBALS[mr];
    $_SESSION[rl_state] = $GLOBALS[state];
    $_SESSION[rl_wa] = $GLOBALS[wa];
    $_SESSION[rl_ay] = $GLOBALS[ay];
    $_SESSION[rl_logged] = $GLOBALS[logged];
    $_SESSION[rl_p_hist] = $GLOBALS[p_hist];
    $_SESSION[rl_mode] = $GLOBALS[mode];
    $_SESSION[rl_cur_page] = $GLOBALS[cur_page];
  }
  ...
  if ($mode == 'brief') {
    ...
    if ($i_id>0) {
      echo "<b>$i_name</b> ($q_num пересказов) <a href=pquiz.php?act=reset&mr=0&i_id=0>Сбросить</a>";
    } else {
      ...
      foreach ($mr_levels_b as $rat => $works) {
        ...
        echo ">".$works." самых известных книг</option>";
      }
      echo "</select>";
    }
  } else {
    echo "Уровень сложности: ";
    if ($i_id>0) {
      echo "<b>$i_name</b> ($q_num книг) <a href=pquiz.php?act=reset&mr=0&i_id=0>Сбросить</a>";
    } else {
      echo "<select name=mr onChange='this.form.submit();'>";
  ...
  if ($a_id>0) {
    if (($correct == $total) && (in_array($total, $prize_cor))) {
      ...
      echo "Вы отгадали <b>$correct</b> писателей подряд из <b>$q_num</b> самых известных книг. Играйте дальше и наберите еще больше правильных ответов подряд!<br>";
      ...
      echo "
        ...
        data-counter='' 
        data-description='Попробуйте отгадать $correct писателей подряд из $q_num книг как я!' 
        data-title='Угадай писателя!' 
        data-url='$share_url'
    ...
    echo "<a href=pquiz.php?nc=$mtime&mr=$mr&mode=$mode&i_id=$i_id>";
    ...
    for ($x=0; $x<$n2; $x++) {
      ...
      echo "<a target=_blank href=works.php?tag=$w2[t_name]>";
    ...
    echo "<a target=_blank href='works.php?w_id=$ww2[w_id]&line=$l'>$ww2[w_name]</a>";
    ...
    if (($logged == 0) && ($w_id>0)) {
      ...
      echo mysql_error();
      if ($i_id>0) {
        mysql_query("UPDATE rl_isles SET i_games=i_games+1 WHERE i_id='$i_id'");
        echo mysql_error();
      }
  ...
  if (($state == 0) || ($w_id == 0)) {
    ...
    if ($i_id>0) {
      ...
      $q = "SELECT * FROM rl_authors WHERE 1=1 $cond3 $year_cond ORDER BY RAND() LIMIT 1";
      $r = mysql_query($q);
      echo mysql_error();
    ...
    if ($mode == 'brief') {
      $cond5 .= " AND b_len>0 AND a_art>0";
    } else {
      // Only prose
      $cond5 .= " AND (t_len>0 OR t_len2>0) AND a_art>1";
    }
  
    $q = "SELECT * FROM rl_works LEFT JOIN rl_authors USING (a_id) WHERE 1=1 AND a_maxr>='$mr' AND w_rating>='$mr' AND w_state<2 $cond2 $cond4 $cond5 $year_cond ORDER BY RAND() LIMIT 1";
    //echo $q;
    $r = mysql_query($q);
    echo mysql_error();
    ...
    if ($i_id>0) {
      $q = "SELECT * FROM rl_authors WHERE a_works>0 $year_cond $cond3 ORDER BY a_year1";
    } else {
      $q = "SELECT * FROM rl_authors WHERE a_works>0 AND a_id != $ww[a_id] AND a_maxr>='$mr' $year_cond $cond3 
  ...
  $min_line = 0;
  if ($mode == 'brief') {
    $st = file_get_contents("brief/$ww[a_id]/$ww[w_id].txt");
    $color='#ffeeff';
  } else {
    if ($ww[t_len]>0) {
      $st = trim(strip_tags(file_get_contents("works/$ww[a_id]/$ww[w_id].txt")));
      $color='#ffffee';
    } elseif ($ww[t_len2]>0) {
      $st = trim(file_get_contents("traum/$ww[a_id]/$ww[w_id].$ww[tr_wtype]"));
...
user-select: none;'><font size=+2>";
  ...
  foreach ($ay as $i => $val) {
    ...
    echo "<td><a href='works.php?act=$mode&w_id=".$wa[$i][w_id]."' target=_blank><img height=22 src=i/help.png></a>";
  }
  echo "</table>";
