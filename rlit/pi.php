<?
  ...
  if ($act == "create") {
    ...
    $q = "SELECT * FROM rl_works WHERE a_id IN ($cond) AND (t_len>0 OR t_len2>0) ORDER BY w_rating DESC";
    $r = mysql_query($q);
    echo mysql_error();
    $n = mysql_numrows($r);
    $need_cut = 0;
    ...
    // Count works (brief)
    $cond = str_replace("-", ",", substr($aids, 1, strlen($aids)-2));
    $q = "SELECT * FROM rl_works WHERE a_id IN ($cond) AND b_len>0 ORDER BY w_rating DESC";
    $r = mysql_query($q);
    echo mysql_error();
    $n = mysql_numrows($r);
    $need_cut = 0;
    ...
    $i_mr = substr($i_mr, 0, -1);
    $i_mr2 = substr($i_mr2, 0, -1);
    $q = "INSERT INTO rl_isles VALUES('', '$aids', '$i_mr', '$i_mr2', NOW(), '$anames', 0)";
    echo $q;
    mysql_query($q);
    echo mysql_error();
    stop_time();
  ...
  if ($act == "new") {
    echo "<b>Выберите писателей для создания острова:</b>";
    if (!isset($order)) $order="a_rating desc, a_year1";
    $q = "SELECT * FROM rl_authors WHERE a_works2>0 ORDER BY $order";
    $r = mysql_query($q);
    echo mysql_error();
    $n = mysql_numrows($r);
  ...
  echo "<th><a href='pi.php?act=new&order=a_prof'>Профессия";
  ...
  echo "<th><a href='pi.php?act=new&order=a_rating desc, a_year1'>Рейтинг";
    ...
    echo "</table>";
    echo "<br>";
    echo "<input type=submit value='Создать остров из выбранных писателей'>";
    stop_time();
    echo "<br>";
    echo "<br>";
    echo "<br>";
  ...
  echo "<p><a href=pi.php?act=new>Создать новый остров писателей</a>";
  echo "<table border=1 cellpadding=3 style='border-collapse: collapse;'>";
  echo "<tr>";
  ...
  $r = mysql_query("SELECT * FROM rl_isles ORDER BY $order");
  echo mysql_error();
  $n = mysql_numrows($r);
  for ($i=0; $i<$n; $i++) {
    $w = mysql_fetch_assoc($r);
    ...
    for ($x=0; $x<$n3; $x++) {
      ...
      if ($good == 1) {
        $st = substr($st, 0, -1);
        echo "<a href='pquiz.php?act=reset&i_id=$w[i_id]&mr=$amm[0]&mode='>$st</a><br>";
      }
      echo "</a> ";
    }
    echo "<td><center>";
    ...
    for ($x=0; $x<$n3; $x++) {
      ...
      if ($good == 1) {
        $st = substr($st, 0, -1);
        echo "<a href='pquiz.php?act=reset&i_id=$w[i_id]&mr=$amm[0]&mode=brief'>$st</a><br>";
      }
      echo "</a> ";
    }
  ...
  echo "<p>Для того, чтобы играть на существующем острове, нажмите на одну из разноцветных ссылок в столбцах 'Книг' или 'Пересказов'";
  stop_time();
?>
